...
dostata is an R interface to Stata that provides the following functionality:
...
This allows you to import and tidy your data in R, then quickly switch to Stata to perform specialised analyses, without leaving your R environment.

## Installation

`dostata` is not currently on CRAN.
You can install the development version from [GitHub](https://github.com/james-atkins/dostata) with:
...
For more information, please read the [Getting Started guide](https://dostata.jamesatkins.com/articles/dostata.html).
...
`dostata` integrates with knitr allowing you to run Stata code chunks in knitr or rmarkdown documents.
To do so, you must first import the package with `library(dostata)` so knitr knows what to do with `stata` chunks.
