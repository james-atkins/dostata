---
title: "Introduction to dostata"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to dostata}
...
The aim of dostata is to enable you to combine the strengths of both R and Stata into one seamless workflow.
This document introduces you to dostata’s basic functionality.
...
<i class="fa fa-info-circle" style="color: #007bff;"></i> **Note: ** All dostata functions start with the prefix `stata_`.

# Finding Stata

In order to run, dostata needs the path to your Stata executable.
...
If needed, you can also run multiple Stata sessions at the same time but most users can use dostata without worrying about session management.

# Transferring Data

## Functions

dostata makes it is easy to transfer datasets between R and Stata.
...
In addition to these direct data transfer functions, dostata provides a more general way to work with data between R and Stata using global macros.
In Stata, global macros of the form `$R_dataframe_name` are used to reference R data frames.
When you create a global macro in this format, dostata automatically exports the specified R data frame to a `.dta` file, which is the file format used by Stata for datasets.
